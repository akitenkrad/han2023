...
//! 決定論的 socsim コア (市場クリアリング・利益計算・解析ベンチマーク・メモリ
//! 更新) からは [`SabmClient`] 型エイリアス経由でのみ触れる．
//!
...
//! するため `sha2` は不要．キャッシュは features=["live"] 経由で有効化される．
//!
//! 設計書 §4.2/§7 は当初 `reqwest` + `sha2` + 手書き `llm.rs` を挙げていたが，
//! 本スイートは li2024 / zhao2024 / chuang2024 と統一して `socsim-llm` クレット
//! (issue #21/#26) に標準化したため `reqwest` / `sha2` は使わず，HTTP とハッシュは
//! socsim-llm が所有する．
//!
//! テストでは `socsim-llm::mock::ScriptedClient` を `Box<dyn LlmClient>` として
//! 同じ [`SabmClient`] に流し込める．
...
pub type SabmClient = CachingClient<Box<dyn LlmClient>>;
...
///   `gpt-4-0314`)．未設定なら空キーのフォールバックを置く (Ollama が成功すれば
///   呼ばれない; 両方失敗時のみ設定エラーになる)．
/// - キャッシュ: `settings.cache_path` があればその JSON ファイル，なければ
///   in-memory．
pub fn build_live_client(settings: &LlmSettings) -> Result<SabmClient, LlmError> {
    // 既定モデルを llama3.2:latest にする (OLLAMA_MODEL 未設定時)．socsim-llm の
    // OllamaClient::from_env 既定は llama3.1 のため，明示的に上書きする (llm_meta.json
    // に記録されるモデル名のバイト等価のため，委譲前に設定する)．
    ...
    socsim_llm::build_live_client(settings.cache_path.as_deref().map(std::path::Path::new))
}

/// 任意の [`LlmClient`] (例: `mock::ScriptedClient`) をキャッシュで包んだ
/// [`SabmClient`] を作る (主にテスト用)．
pub fn wrap_client<C: LlmClient + 'static>(backend: C, cache: PromptCache) -> SabmClient {
