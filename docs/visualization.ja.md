...
```bash
uv run sabm-tools reproduce                  # 直近の *_reproduce 結果を読む
uv run sabm-tools reproduce --run --mock --quick   # 先に Rust バイナリをオフライン実行
```

`reproduce_summary.json` とシナリオごとの `rounds.csv` / `metrics.csv` を読み，PASS/off アンカーテーブルを表示し，3 つの図を `<results>/figures/` に描画する: `fig1_no_communication.png` (会話なしの価格軌跡)・`fig2_communication.png` (会話あり変種)・`fig4_collusion_compare.png` (会話なし vs 会話あり の collusion index 比較)．`--run` は先に `sabm reproduce` を実行する (`--mock` / `--quick` / `--seed` を透過)．`--json` は図を描かずサマリのみ出力する．
